...
void Parser::generate_object_tree(std::vector<ClassDefinition*>& cdefs)
{
	...
	*/
	...
	ObjectTree joao; //Wow, it's the real Joăo Gaming!
	...
	for (; where <= there; ++where)
	{
		...
		case(Token::cEnum::KeywordToken):
		{
			...
			case(KeywordToken::Key::For):
			{
				...
				ASTNode* cond = readExp(where, static_cast<int>(semicolon)); // Assignments do not evaluate to anything in Joăo so putting one in a conditional is silly
				where = static_cast<int>(semicolon + 1);
				ASTNode* inc;
